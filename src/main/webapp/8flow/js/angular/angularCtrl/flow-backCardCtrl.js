flowApp.controller('backCardController', function($rootScope, $routeParams,$scope,$cookieStore,$HttpService,$location) {
	...
	$scope.confirmBack = function(){
		$scope.orders = [];
		for(var i = 0; i < $scope.backInfo.backCardList.length; i++){
			var backOrder = $scope.backInfo.backCardList[i];
			...
			console.log($scope.backInfo.backCardList[i]);
			console.log($scope.backInfo.backCardList[i].orderNo);
			var obj = {
			}
	...
	$scope.init = function(){
		...
		var postValue = {
			...
			$HttpService.post(postValue).then(function(res) {
				...
				if(res.data.resultCode == 1){
					$scope.backInfo = res.data.resultBody;
